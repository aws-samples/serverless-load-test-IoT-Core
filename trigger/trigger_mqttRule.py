...
# mqtt configuration
ruleTopic = 'loadtest/lambda/mqtt'
mqtt_connection = 0
count = 0
...
def publish_mqttSingle():
    global count
    print("Time: ", datetime.datetime.now(datetime.UTC))
    for m in range(1):      # 每次发送1个trigger
        count +=1
        # single publish request:
        message_topic = ruleTopic
        message_json = {"type": "trigger","triggerNo": "13", "ts":""}
        message_json["triggerNo"] = str(count)
...
if __name__ == '__main__':
    try:
        ...
        # repeat publishing
        n =  331       # 每1秒发送一次mqtt publish, 持续 n 秒, n = 1800, 10, 100, 200, 300
        schedule.every().second.until(timedelta(seconds=n)).do(publish_mqttSingle)  # 每1秒
        while True:
